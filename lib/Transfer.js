...
class Transfer extends BaseRpc {
   ...
   *
   * 广播
   * @param {string} unsignTx 交易hash
   * @returns {Promise<object>} result 返回结果
   * @memberof Transfer
   */
  async broadcast(unsignTx) {
    try {
      const result = await this.seroRpc.post(
        '',
        Object.assign(this.httpData, { method: 'exchange_commitTx', params: [unsignTx] })
      );
      this.httpSuccess(result);
      return result.data;
    } catch (error) {
  ...
  async getTransactionReceipt(txHash) {
    try {
      const result = await this.seroRpc.post(
        '',
        Object.assign(this.httpData, { method: 'sero_getTransactionReceipt', params: [txHash] })
      );
      this.httpSuccess(result);
      return result.data;
    } catch (error) {
