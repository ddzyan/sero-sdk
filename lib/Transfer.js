...
class Transfer extends BaseRpc {
  ...
  async exchangeCreateTx({ Gas = 25000, GasPrice = 1000000000, From, RefundTo, Receptions, Roots }) {
    try {
      const result = await this.seroRpc.post('', {
  ...
  async exchangeBroadcast(unsign) {
    try {
      const result = await this.seroRpc.post('', { method: 'exchange_commitTx', params: [unsign], jsonrpc: this.jsonrpc, id: this.id });
      this.httpSuccess(result);
      return result.data;
    } catch (error) {
      this.httpError(error);
