...
class Account extends BaseRpc {
  ...
  createAccount(text) {
    ...
    return { sk, tk, pk, tk_base58, pk_base58, sk_base58 };
  }
  ...
  generatePKr(pkStr) {
    ...
    return { pkr, rnd: rnd.toString('hex') };
  }
  ...
  async getRecords(start_number, end_number, pk) {
    try {
      const result = await this.seroRpc.post(
        '',
        Object.assign(this.httpData, { method: 'exchange_getRecords', params: [start_number, end_number, pk] })
      );
      this.httpSuccess(result);
      return result.data;
    } catch (error) {
  ...
  async clearUsedFlag(pk) {
    try {
      const result = await this.seroRpc.post(
        '',
        Object.assign(this.httpData, { method: 'exchange_clearUsedFlag', params: [pk] })
      );
      this.httpSuccess(result);
      return result.data;
    } catch (error) {
